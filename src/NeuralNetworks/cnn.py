...
def bias_variable(shape):
    bias = tf.random_normal(shape, mean=0., stddev=0.01)
...
class CNN():
    def __init__(self):
        ...
        self.weights = {
            'w_conv_1': tf.Variable(tf.random_normal([self.n_hidden, self.n_classes])),
            'w_conv_2': tf.Variable(tf.random_normal([self.n_hidden, self.n_classes])),
            'w_pooling': tf.Variable(tf.random_normal([self.n_hidden, self.n_classes]))
        }
        self.biases = {
            'out': self.init_bias
